...
- `examples/nonreturning/` — F9 FIXTURE **and its own negative control**: sealed `Latch permits Idle, Armed, Fired`, dispatched from `LatchMachine.step`. Two helpers both return `Latch` and both defeat the shallow `collectReturns` walk (which does not descend into a `switch`), so only the presence of a `return` separates them. `reject` has none — provably always throws, so its three arms contribute **no edge**. `escalate` hides a `return` inside a `switch` — it can produce a state the analysis cannot see, so its arm must stay **UNRESOLVED**. Dropping that one is the false-drop F9 must never cause; it is the whole risk of the rule and the reason the fixture pairs them. 3 states, 4 transitions, 3/4 resolved, `Fired` terminal.
